...
def grafico(f, c):
    ...
    try:
        # -------- Lê o arquivo das Medidas de Luminosidade --------
        with open("medidas_luminosidade.txt", 'r') as dados:
            for line in dados:
                line = line.strip()
                if line != "":
                    try:
                        valores_luminosidade.append(float(line))
                    except:
                        pass

        # -------- Lê o arquivo das Médias das Medidas de Luminosidade  --------
        with open("media_luminosidade.txt", 'r') as medias:
            for line in medias:
                line = line.strip()
                if line != "":
                    try:
                        luminosidade_media.append(float(line))
                    except:
                        pass
        ...
        # Plota os dados de intensidade luminosa
        axis1.plot(valores_luminosidade, label='LUMINOSIDADE', color='blue')

        # Plota as medidas de Média Móvel running average
        axis1.plot(luminosidade_media, label='MÉDIA LUMINOSIDADE', color='red')
...
raiz.title("MEDIDAS LUMINOSIDADE") #funcao para alterar titulo da janela
