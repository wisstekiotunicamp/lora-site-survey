...
def grafico(f, c):
    ...
    try:
        # -------- Lê o arquivo das Medidas de Umidade --------
        with open("medidas_umidade.txt", 'r') as dados:
            for line in dados:
                line = line.strip()
                if line != "":
                    try:
                        valores_umidade.append(float(line))
                    except:
                        pass

        # -------- Lê o arquivo das Médias das Medidas de Umidade  --------
        with open("media_umidade.txt", 'r') as medias:
            for line in medias:
                line = line.strip()
                if line != "":
                    try:
                        umidade_media.append(float(line))
                    except:
                        pass
        ...
        axis1.plot(valores_umidade, label='UMIDADE', color='blue')

        # Plota as medidas de Média Móvel running average
        axis1.plot(umidade_media, label='MÉDIA UMIDADE', color='cyan')

        axis1.legend()
        axis1.set_ylabel('UMIDADE (%)')
...
raiz.title("MEDIDAS UMIDADE") #funcao para alterar titulo da janela
